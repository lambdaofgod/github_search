...
def plot_similarity_heatmap(similarity_df, output_dir):
    ...
    plt.axvline(similarity_df['jaccard_similarity'].mean(), color='red', linestyle='--', 
                label=f'Mean: {similarity_df["jaccard_similarity"].mean():.2f}')
    plt.axvline(similarity_df['jaccard_similarity'].median(), color='green', linestyle='--', 
...
def plot_top_nodes_comparison(centrality_df, output_dir, top_k=10):
    ...
    for repo in repositories:
        ...
        for i, measure in enumerate(measures):
            measure_df = repo_df[repo_df['measure'] == measure].sort_values('centrality_score', ascending=False).head(top_k)
...
def main():
    ...
    # Plot top nodes comparison
    print("Creating top nodes comparison plots")
    plot_top_nodes_comparison(centrality_df, args.output_dir, top_k=args.top_k)
    print(f"Saved top nodes comparison plots to {args.output_dir}")
