...
def prepare_data(upstream, product):
    ...
    datasets_df = pd.read_json(
        upstream["sentence_embeddings.prepare_paperswithcode_data"]["datasets"]
    )
    methods_df = pd.read_json(
        upstream["sentence_embeddings.prepare_paperswithcode_data"]["methods"]
    )
    dbpedia_df = dbpedia.normalize_dbpedia_df(dbpedia_df)
    datasets_df = paperswithcode.clean_datasets_df(
        paperswithcode.normalize_datasets_df(datasets_df)
    )
    methods_df = paperswithcode.clean_methods_df(
        paperswithcode.normalize_methods_df(methods_df)
    )
