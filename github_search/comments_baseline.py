...
__all__ = ['get_comment_contents', 'bm25_tokenizer', 'get_tokenized_comments', 'get_valid_comments_and_files',
           'run_dask_series_apply', 'RankBM25AggregateSearcher']
...
def get_comment_contents(file_contents):
    ...
    comments = [
        tok.replace("#", '')
        for (toktype, tok, _, _, _) in tokenize.generate_tokens(io.StringIO(file_contents).readline)
...
def get_valid_comments_and_files(python_files_df, comment_contents):
    ...
    tokenized_comments = run_dask_series_apply(comment_contents.dropna(), bm25_tokenizer)
...
class RankBM25AggregateSearcher:
    ...
    def get_aggregate_scores(self, df, indices, top_scores, aggregated_column='repo_name'):
