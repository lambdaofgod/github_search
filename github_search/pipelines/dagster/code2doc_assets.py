...
@multi_asset(
    outs={"dependency_signature_generated_readmes": AssetOut()},
    ins={"repos_with_dependency_signatures_df": AssetIn()},
)
def code2doc_readmes_from_dependency_signatures(
    context: AssetExecutionContext,
    config: Code2DocModelConfig,
    ...
    """
    README files generated by Code2Doc from dependency signatures - uses call graph analysis
    """
    code2doc_config = config
    logging.info(
        f"Generating readmes with code2doc from dependency signatures using {code2doc_config.lm_model_name}, "
        f"using maximum of {code2doc_config.files_per_repo} files per repo"
    )
    lm = dspy.LM(
        model="ollama_chat/" + config.lm_model_name, base_url=config.lm_base_url
    ...
    if config.is_debug_run:
        repos_with_dependency_signatures_df = repos_with_dependency_signatures_df.head()

    with tqdm_logging_redirect():
        generated_readme_df = run_code2doc_on_df(
            repos_with_dependency_signatures_df,
            code_col="dependency_signature",
            name_col="repo",
        )
    context.add_output_metadata(
        {
        ...
        output_name="dependency_signature_generated_readmes",
    )
    yield Output(
        generated_readme_df,
        output_name="dependency_signature_generated_readmes",
...
@multi_asset(
    outs={"flat_dependency_signature_generated_readmes": AssetOut()},
    ins={"repos_with_dependency_signatures_df": AssetIn()},
)
def code2doc_flat_readmes_from_dependency_signatures(
    context: AssetExecutionContext,
    config: Code2DocModelConfig,
    repos_with_dependency_signatures_df: pd.DataFrame,
    phoenix: PhoenixTracker,
    ...
    """
    README files generated by Code2Doc Flat from dependency signatures - single-step direct dependency-signature-to-documentation generation
    """
    code2doc_config = config
    logging.info(
        f"Generating readmes with code2doc FLAT from dependency signatures using {code2doc_config.lm_model_name}, "
        f"using maximum of {code2doc_config.files_per_repo} files per repo"
    )
    lm = dspy.LM(
    ...
    context.add_output_metadata(
        ...
        output_name="flat_dependency_signature_generated_readmes",
    )
    yield Output(
        generated_readme_df,
        output_name="flat_dependency_signature_generated_readmes",
